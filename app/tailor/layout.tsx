...
export default function TailorLayout({
  ...
  React.useEffect(() => {
    ...
    const uniqueChannelName = `global-tailor-messages-${userId}-${Math.random().toString(36).substring(7)}`
    ...
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      ...
      <header className="md:hidden sticky top-0 z-40 w-full border-b border-border bg-background/80 backdrop-blur-md h-16 flex items-center justify-between px-4">
        <Link href="/" className="flex items-center space-x-2">
          ...
          <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">Tailor</span>
        </Link>
      ...
      >
        <div className="flex flex-col flex-1">
          ...
          <div className="p-4 border-b border-border/60">
            <div className="flex items-center space-x-3 p-2 bg-muted/40 rounded-2xl border border-border/50">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-foreground truncate">{tailorName}</p>
                <p className="text-[10px] text-primary font-semibold flex items-center gap-0.5 mt-0.5">
                  <Sparkles className="w-3 h-3 text-rust" aria-hidden="true" />
                  <span>Verified Tailor</span>
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Nav links */}
          <nav className="flex-1 p-4 space-y-1" aria-label="Tailor workspace navigation">
            {menuItems.map((item) => {
